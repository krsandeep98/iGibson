...
def dry_run_base_plan(env, path, path1, time_to_apply_the_path):
        ...
        if path is not None and path1 is not None:
            ...
            print("length of paths, number of waypoints 2 when agent is none",  len(path1))
            ...
            for way_point1 in path1:
                ...
                if time()- t0 > time_to_apply_the_path:
                    ...
                    # print("waypoint for the agent, waypoint_obs, waypoint counter", way_point, way_point1, waypoint_counter_fetch)
                    return path, path1[waypoint_counter_turtlebot:]
            # print("path/path1 is not found, and setting the agent to its initial position")
            # if path is None:
            return path, path1[waypoint_counter_turtlebot:] #need to add this to keep things consistent with this function returning the path left to execute for all robots
...
def main():
    ...
    for j in range(10):
        ...
        print("TIMEEEEEEEEEEEEEE",exec_time, plan_time, tau_s, tau_l, t_cutoff)
        # env.reset()
        # for i in range(100):
        if j != 0:
            env.reset()
        ...
        print("distance between current and goal before reference run", distance_fn(env.robots[0].get_position(), env.task.target_pos))
        ...
        while distance_fn(env.robots[0].get_position(), env.task.target_pos) >= 0.01 and time()-start_time < t_cutoff and ref_rrg is not None:
            ...
            # new_path_for_agent, collision2 = plan_base_motions_update(env, ref_rrg, updated_agent_path, env.task.target_pos, 0, maxtime=tau_s/2)#tau_s
            new_path_for_agent, collision2 = plan_base_motions_update(env, ref_rrg, path_for_base, env.task.target_pos, 0, maxtime=plan_time)#tau_s/2
            if collision2 == 0:
        ...
        if collision2 ==0 or collision2 ==1 or collision1:
        ...
        print('time elapsed {:.4f}, collision_counter {}, while_loop number {}, short_interval success counter {}, Iteration {}, avg time {:.4f}, COUNTER {}'
                .format(time_elapsed, collision_counter, while_loop_counter, short_interval_success_counter, j+1, total_time_taken/(j+1), counter))
