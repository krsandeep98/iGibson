...
if __name__ == "__main__":
    ...
    # print("results of unpicklable function call", get_unpicklable(env))
    ...
    for j in range(1):        
        ...
        with Pool(processes=4) as pool:         # start 4 worker processes
            result = pool.apply(multiproc_dry_run_agent, (env, path_for_base))#remote gives an error about not able to pickle the fn, not being the same object
            result.get(timeout=3)
        
        # with concurrent.futures.ProcessPoolExecutor(4) as executor:
        #     result = [executor.submit(multiproc_dry_run_agent, (env, path_for_base))]
        #     concurrent.futures.wait(result)
        
        # p1 = multiproc_dry_run_agent.remote(env, path_for_base)
        # processes.append(p1)
        # p2 = multiproc_dry_run_obstacle.remote(env, path_for_base1)
        # processes.append(p2)

        # ray.get(processes)

        # p1 = env.multiproc_dry_run_agent.remote(env, path_for_base)
        # processes.append(p1)
        # p2 = env.multiproc_dry_run_obstacle.remote(env, path_for_base1)
        # processes.append(p2)
