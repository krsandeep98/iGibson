...
def rrg(start, goal, distance, sample, extend, collision, radius=0.5, max_time=INF, max_iterations=INF, goal_probability=.2, informed=True):
    ...
    #     print("start or goal is in collision in rrg call")
    #     return None, True
    # print("distance between start and goal in rrg call", distance(start, goal))
    ...
    while (time() - t0) < max_time and it < max_iterations:
        ...
        if do_goal and distance(new.config, goal) < 1e-2:
            # print("distance between new and goal", distance(new.config, goal))
            # if distance(new.config, goal) < 1e-2:
            print("YAY!! distance between start and goal in rrg call", distance(start, goal))
            print("goal_n is being set in rrg call", it)
            goal_n = new
        ...
        for n in neighbors:
            ...
            if n.cost + d < new.cost:#this is not needed in the rrg step as we add all the neighbors to the new node
                ...
                if len(path) != 0 and distance(new.config, path[-1]) < 1e-2:
    ...
    if goal_n is None:
        print("NO!!! distance between start and goal in rrg call", distance(start, goal))
        print("goal_n is none in rrg call")
        return None, False
