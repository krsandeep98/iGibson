...
def safe_path_update(sequence, collision):
    path = []
    safety_counter = 1000 #100 restricts the length of the path later on since it depends on the safety counter
    # safety counter to be length of seq would give init coll in error if the sequence is of 0 length
...
class rrt_dynamic(object):
    ...
    def rrt(self, start, radius=0.5, max_time=INF, max_iterations=INF, goal_probability=.2, informed=True):
        ...
        while (time() - t0) < max_time:
            ...
            # for n in neighbors:  # TODO - avoid repeating work
            #     d = self.distance(new.config, n.config)
            #     if new.cost + d < n.cost:
            #         path = safe_path(self.extend(new.config, n.config), self.collision_static)
    ...
    def rrt_update_2paths(self, start, ref_path, max_time=INF, max_iterations=INF, goal_probability=.2, informed=True):
        ...
        if ref_path is None or len(ref_path)==0:
            ...
            # this loop has to plan for the waypoints (valid) to goal if the goal is still valid
                ...
                self.reference_traj = ref_path
                self.next_traj = updated_path
                # print ("setting the next traj in update function to later one only", len(self.next_traj))
                # print ("setting the reference path in update function to later one only", len(self.reference_traj))
                # return True
                return 4
