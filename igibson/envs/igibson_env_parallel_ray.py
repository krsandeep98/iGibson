...
from igibson.external.pybullet_tools.utils import pairwise_collision, set_base_values, plan_base_motion, set_base_values_with_z
...
class iGibsonEnv(BaseEnv):
    ...
    def plan_base_motion_dynamic(self, goal):
        robot_number = 0
        print("start dynamic obstacle", (self.task.dynamic_objects[robot_number].get_position()))
        ...
        yaw = self.task.dynamic_objects[robot_number].get_rpy()[2]
        occupancy_range = self.sensors["scan_occ"].occupancy_range
        half_occupancy_range = occupancy_range / 2.0
        robot_position_xy = self.task.dynamic_objects[robot_number].get_position()[:2]
        corners = [
        ...
        mp_obstacles = []
        mp_obstacles.append(self.scene.mesh_body_id)# for the static scenes
        mp_obstacles.append(self.robots[0].robot_ids[0])
        ...
        path, coll_or_not = plan_base_motion(
            ...
            self.task.dynamic_objects[robot_number],
            self,
            [x, y, theta],
    ...
    def plan_base_motions(self, goal):
        robot_number = 0
        print("start FETCH", (self.robots[robot_number].get_position()))
        ...
        yaw = self.robots[robot_number].get_rpy()[2]
        occupancy_range = self.sensors["scan_occ"].occupancy_range
        half_occupancy_range = occupancy_range / 2.0
        robot_position_xy = self.robots[robot_number].get_position()[:2]
        corners = [
            robot_position_xy + rotate_vector_2d(local_corner, -yaw)
            for local_corner in [
                np.array([half_occupancy_range, half_occupancy_range]),
        ...
        mp_obstacles = []
        mp_obstacles.append(self.scene.mesh_body_id)# for the static scenes
        mp_obstacles.append(self.task.dynamic_objects[0].robot_ids[0])
        # mp_obstacles.extend(env.scene.get_body_ids()) #for the interactive ones

        # env.base_mp_resolutions = np.array([0.05, 0.05, 0.05])
        path, coll_or_not = plan_base_motion(
            # env.robots[robot_number].robot_ids[0],
            self.robots[robot_number],
            self,
            [x, y, theta],
            (tuple(np.min(corners, axis=0)), tuple(np.max(corners, axis=0))),
            # map_2d=grid,
    ...
    @ray.remote
    def multiproc_dry_run_agent(self, path):
        t0 = time.time()
        if path is not None :
            ...
            for way_point in (path):
                print("setting AGENT position with waypoints", waypoint_counter_fetch)
                set_base_values_with_z(
                    self.robots[0].robot_ids[0], [way_point[0], way_point[1], way_point[2]], z=self.initial_pos_z_offset
                )

                waypoint_counter_fetch+=1
                self.simulator.step()#or sync
                # if time() - t0 > 2:
                #     print("timeout in the agent dry run function", waypoint_counter_fetch)
                #     break
            print("total time taken for the AGENT path exec", time.time()-t0)

    @ray.remote
    def multiproc_dry_run_obstacle(self, path):
        t0 = time.time()
        if path is not None :
            # if self.mode in ["gui", "iggui", "pbgui"]:
            ...
            for way_point in (path):
                ...
                set_base_values_with_z(
                    self.task.dynamic_objects[0].robot_ids[0], [way_point[0], way_point[1], way_point[2]], z=self.initial_pos_z_offset
                )
                ...
                #     break
            print("total time taken for the obstacle path exec", time.time()-t0)
    
    def get_current_position(self):
        print("current position of robot after reference traj applied ", self.robots[0].get_position())
        print("current position of turtlebot after reference traj applied ", self.task.dynamic_objects[0].get_position())
